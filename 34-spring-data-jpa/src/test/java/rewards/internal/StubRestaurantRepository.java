...
/**
 * A dummy restaurant repository implementation. Has a single restaurant "Apple
 * Bees" with a 8% benefit availability
 * percentage that's always available.
 * 
 * Stubs facilitate unit testing. An object needing a RestaurantRepository can
 * work with this stub and not have to bring
 * in expensive and/or complex dependencies such as a Database. Simple unit
 * tests can then verify object behavior by
...
public class StubRestaurantRepository implements RestaurantRepository {
	...
	/**
	 * A simple "dummy" benefit availability policy that always returns true. Only
	 * useful for testing--a real
	 * availability policy might consider many factors such as the day of week of
	 * the dining, or the account's reward
